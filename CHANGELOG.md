...
### 보안
- Control-plane 대시보드의 JSON 렌더링 경계를 보강했습니다. 스캔 count는 숫자로 강제하고 `data-id`는 HTML 속성 이스케이프를 적용하며 severity 색상은 고정 palette의 own property만 허용합니다. 기존 dashboard security contract가 이 positive/negative 경계를 고정합니다.
