...
- Claude plugin nested shell 검출 보강 — 직접 `sh`/`bash`/`dash`/`ksh`/`zsh`의 `-c` option payload와 구조화 shell argv를 bounded하게 재검사합니다. Reporting·assignment·wrapper·comment prose와 `$0…` 후속 인자는 실행 payload로 합치지 않으며, `apply-now`·`install-chart` 같은 near-verb도 제외합니다.
