...
### 보안
- Exact-head review-control 증거를 갱신했습니다. #1192의 dashboard 회귀는 유지되고 8개 repository/security workflow가 GREEN이지만 CodeQL authenticated verdict와 browser 증거는 incomplete입니다. #999는 9개 repository workflow가 GREEN이나 Required Noema가 `orchestrator/free` 다중 429/timeout 뒤 HTTP 502로 실패했습니다. Canonical owner contextual-orchestrator #1094의 protected merge·immutable release·consumer bump 전에는 review 실패를 `Clean Scan`이나 승인으로 바꾸지 않습니다.
