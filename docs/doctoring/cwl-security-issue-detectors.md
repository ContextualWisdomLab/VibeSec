...
| Secret indirection / auth comments | SAST | #1106 | this successor | implements regression lock on existing `_scan_file` rules |
| Claude plugin supply chain | SAST | #1099 | this successor | implements `claude-plugin-*` |
...
(MITRE, n.d.-c). GitHub Actions workflow identities persist after file deletion
