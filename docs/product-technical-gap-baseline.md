...
| #1157, `feat/claude-plugin-conflicting-identity-1099` | Predecessor `6f0eb445ecc0d48dfafd2c3d2525eb7cc1e462f7`; exact repair head `ba38201644b9ffa75721e14eb56dc4f4c2c9ef5e`; source blob `e2e0a02a1fc64bee595826d6681a665aa6fc16b5`; base `a6f6a742701c263efcd83402da1815e065d69706`; open/Draft, four ahead / zero behind. The repair is one normal descendant changing one production file. | One global `seen` set erased runtime namespace: a plugin namespace equal to a local Skill, or an Agent equal to a Skill, became a false collision. The minimal repair keys seen NFC identities by `plugin`, `skill` (including legacy Commands), `command`, or `agent`; same-namespace duplicates remain findings. | Direct predecessor RED: cross-namespace negatives 2 failed while duplicate-Skill positive passed. Exact source GREEN: focused existing corpus plus causal probes 17/17 and `py_compile` pass. This isolated harness stubs an unrelated scanner adapter; no full-suite or hosted result is transferred. Custom-base PR currently has no hosted run, formal review, or thread; keep Draft pending parent integration, exact-head Checks, independent approval, protected merge, and release. |
...
| Canonical central CodeQL owner #2040 and related #2051 | `.github#2040` comments `5644656829` and `5644947368` bind attempt 1 and the terminal-failure attempt-2 canary. Fresh owner head is Draft `6476b919d3febf79cc53e71d6d60f15d7e83ced4` on `main@cb0872c9a20d5584703dffacca65c096fc034c6c`. It is an ordinary successor that restores the complete scheduler test module after transport truncation on predecessor `be25a767...`; the PR body still names `a9b18b4b...`, so that body state is stale. #2051 separately describes a proposed new-client base-ref title/bootstrap mismatch. | This consumer uses the protected old title form; terminal producer-job failure still does not prove the distinct #2051 mismatch. The owner head currently has no pull-request workflow run, no approval, and no unresolved inline thread on the observed generation. | Keep #2040 Draft until its body/evidence is current, mergeability is restored, exact-head hosted gates pass, and qualifying independent review exists. Then integrate normally, release an immutable contract, and validate consumers; no release is claimed. |
...
Keep #1036 Ready for independent review on unchanged `832d066...`; do not merge while CodeQL and current-head approval are incomplete. Keep #1157 Draft on `ba382016...` until its custom-base candidate receives hosted exact-head Checks, parent integration, and independent approval; do not transfer the isolated 17-test result as repository-wide evidence. Keep #104 Draft on unchanged `4d738ccc...`: its HTTP-readiness functional repair is GREEN, but cancelled positive-LSM evidence and absent qualifying approval prohibit integration or release. Keep the central CodeQL canary at #2040 and the runner/workflow/queue handoff at #1590/#2083/#712 rather than starting competing owners or adding no-op triggers. Continue another safely owned reproducible corpus obligation while central work proceeds.
