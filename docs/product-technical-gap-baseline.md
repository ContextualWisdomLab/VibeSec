...
3. Keep #1068 on source-neutral exact head `5897711198b148ae5e5b85ff4308ff0b4d387487` as the single Draft hostless/unresolved-DNS lane. Six repository workflows are GREEN; CodeQL exact logs show successful dispatch followed by the fail-closed `VERDICT_STATE=pending` handoff, Security Scan and SAST Semgrep are queued, no independent approval exists, and one detector thread remains unresolved; #1128 is retired only by verified complete carryover.
