...
This refresh verifies the #1036 JSON serialization repair and its current CodeQL/review boundary. It does **not** relabel every earlier repository/PR observation as current.
...
| G-07 | Remove shared required-review/security capacity and settlement failures. | Canonical `.github` / contextual-orchestrator owners | Still open. #2040 now has the exact consumer canary; require authentic terminal scan evidence, owner GREEN/release and consumer validation. |
...
Keep #1036 Ready for independent review on unchanged `832d066...`; do not merge while CodeQL and current-head approval are incomplete. Keep the central CodeQL canary at #2040 rather than starting a competing owner or adding a no-op trigger. Continue another safely owned reproducible corpus obligation while central work proceeds.
