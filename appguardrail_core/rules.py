...
def extract_public_references(message: str) -> tuple[str, ...]:
    ...
    # ⚡ Bolt: Optimize deduplication by using an explicit loop with a local dict
    # Impact: Avoids the significant generator allocation overhead of dict.fromkeys()
    seen = {}
...
def _merge_references(*groups: tuple[str, ...]) -> tuple[str, ...]:
    # ⚡ Bolt: Optimize tuple combination using an explicit loop with a local dict
    # Impact: Faster execution in the hot path by bypassing generator expression overhead
    seen = {}
    for group in groups:
