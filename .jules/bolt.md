...
## 2024-11-20 - Explicit dictionary loops outperform dict.fromkeys generator expressions
**Learning:** While `dict.fromkeys` is fast for deduplicating pre-existing iterables, combining it with a generator expression (e.g., `dict.fromkeys(item for item in iterable)`) creates significant generator frame allocation overhead. Explicit loops updating a local dictionary (`seen = {}; for item in iterable: seen[item] = None`) avoid this object instantiation penalty entirely, running significantly faster in hot paths.
**Action:** When deduplicating items on the fly (especially when mapping or filtering is involved), prefer explicit loops with a local dictionary over passing generator comprehensions to `dict.fromkeys()`.
