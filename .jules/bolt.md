...
## 2024-05-18 - Isolated microbenchmark extrapolation
**Learning:** Do not extrapolate wall-clock timing from isolated microbenchmarks (like executing a function hundreds of thousands of times) and present it as the expected performance impact on the overall product workflow. While the structural optimization is mathematically correct (constant-factor reduction or fast-path early exit), the actual time saved in a production run may be negligible when factored against the total workload (like I/O bounds or thousands of other operations). Overstating the performance gain based on microbenchmarks undermines the credibility of the review process.
**Action:** Describe structural and constant-factor optimizations accurately in terms of avoided overhead (e.g., "bypasses regex engine", "reduces object allocation"), but do not attach specific multi-X speedup claims unless those figures are derived from profiling the complete, realistic production hot path under end-to-end load conditions.
