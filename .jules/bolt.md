...
## 2024-07-25 - Skip regex execution using substring check for finding tags
**Learning:** `extract_public_references` uses `re.finditer` to locate standard bracketed tags like `[OWASP...]` and `[CWE...]` in finding messages. For the vast majority of findings that don't include these tags or where the message is empty, initializing and executing the regex engine incurs significant unnecessary overhead (~5us per call) when parsing thousands of findings.
**Action:** When evaluating regexes that strictly depend on the presence of a specific character (like `[` for tags), introduce a fast-path substring check (e.g., `if not message or "[" not in message: return ()`). This C-optimized native string membership test reduces execution time for non-matching strings by nearly 90% (~0.7us).
