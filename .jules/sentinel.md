...
## 2026-09-07 - SSRF Defense in Depth
**Vulnerability:** Inconsistent SSRF validation where user-provided webhook URLs were only validated in the HTTP handler, leaving the internal `set_webhook` function vulnerable to bypassed validation if called directly or from other entry points.
**Learning:** Security validation parsing URLs must be enforced at the storage boundary prior to database insertion.
**Prevention:** Delegate all security validation (like `_is_safe_url` for SSRF) to the storage boundary function prior to database insertion. Ensure empty strings are normalized to `None`. Catch `ValueError` from the storage function in HTTP handlers rather than duplicating validation.
