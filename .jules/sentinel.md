...
## 2024-05-24 - Delegating SSRF Validation to Storage Boundary
**Vulnerability:** A Stored SSRF vulnerability pattern (CWE-918) was found where webhook URL validation (`_is_safe_url`) was handled at the API route layer rather than inside the core state-mutation boundary (`set_webhook`). This allowed potential internal bypassing if `set_webhook` was called from other internal contexts without validation.
**Learning:** Security validation must be enforced at the lowest possible layer (the boundary where state changes are committed) rather than relying on upper layers (like API handlers) to remember to validate inputs.
**Prevention:** Always place input validation (like URL parsing and SSRF checking) directly within the function that persists the data to the database, ensuring all entry points are protected. Let the core function throw a `ValueError` for the handler to catch and translate into a 400 Bad Request.
