...
## 2024-05-24 - SSRF Defense-in-Depth Validation
**Vulnerability:** A defense-in-depth opportunity was identified for Stored SSRF (CWE-918). While the actual outbound network sink (`_send_alert`) properly enforced `_is_safe_url` at the time of execution, the initial URL persistence (`set_webhook` via `/api/v1/webhook`) lacked early validation, allowing potentially unsafe URLs to enter the system state before being blocked later.
**Learning:** While the canonical network boundary (the outbound sink) is the ultimate authority for preventing SSRF and DNS rebinding (TOCTOU), enforcing a secondary check at the API/storage boundary provides defense-in-depth. It fails fast for users and maintains invariant guarantees about data at rest.
**Prevention:** Implement defense-in-depth by placing invariant validation (like `_is_safe_url`) at early API boundaries (e.g., in `/api/v1/webhook` or `set_webhook`), while always keeping the strict canonical SSRF and TOCTOU protections at the final execution sink (`_send_alert` and `SafeRedirectHandler`).
