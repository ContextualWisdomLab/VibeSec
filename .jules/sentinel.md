...
## 2025-02-27 - Dashboard DOM XSS and Prototype Pollution Mitigation
**Vulnerability:** Found unsanitized variables originating from JSON payloads (`s.id`, `s.deploy_blocking`, `f.severity`) being injected directly into the HTML using `.innerHTML`, introducing Stored XSS via HTML attribute injection (`data-id="${s.id}"`) and a Prototype Pollution path via direct object key lookup (`SEV[f.severity]`).
**Learning:** Even though the frontend receives API responses, these JSON values could have originated from attacker-controlled repository states or payload injections. The reliance on template literals (`innerHTML`) without strict numeric typing or string escaping created an implicit trust in the structure of the JSON payload. Furthermore, relying on unmodified object property accessors using untrusted strings opens a path to manipulating the object's prototype.
**Prevention:** Strictly enforce type coercion (`Number()`) for expected numeric fields before interpolating into HTML, explicitly sanitize string inputs (e.g., `esc()`), and sanitize keys (e.g., `String(val).toUpperCase()`) prior to object property lookups.
## 2025-02-27 - Dashboard Prototype Pollution Edge Case Contract
**Vulnerability:** A hidden CI test contract asserted that `SEV[String(f.severity).toUpperCase()]` (used to lookup severity colors) could still theoretically be bypassed if an attacker managed to mutate global built-in properties (which might override default array behaviors) or pass a key that resolves to prototype properties (e.g. `__proto__`, `constructor`), leading to DOM manipulation via CSS variable injection.
**Learning:** Even coercing input values to uppercase strings before an object lookup isn't fully foolproof against Prototype Pollution if the property isn't explicitly validated to exist as an *own* property on the dictionary object.
**Prevention:** Use `Object.hasOwn(obj, key)` (or `Object.prototype.hasOwnProperty.call`) before accessing dynamically requested keys on configuration objects to definitively block prototype pollution payloads.
## 2025-02-27 - Dashboard Count Domain Evasion Mitigation
**Vulnerability:** Numerical constraints enforced superficially with `Number(val) || 0` are not true count-domain contracts. Threat models that can pass strings like `"1e309"` can induce rendering regressions (producing `Infinity`) or bypass downstream validations assuming safe, finite integers. Similarly, negative numbers pass `Number()` successfully.
**Learning:** `Number()` alone is inadequate when business logic requires finite, safe, and positive discrete counts. Fractional, scientific notation, negative, and oversized values must be guarded against.
**Prevention:** For UI metric projections, use parsing and validation combinators `parseInt(val, 10)` along with `Number.isSafeInteger(val)` and `val >= 0`. Provide this unified parsing strategy to stats, history arrays, accessibility pills, and geometry trend blocks uniformly.
