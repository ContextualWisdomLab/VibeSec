...
## 2024-05-18 - [CRITICAL] SSRF bypass via empty hostname in URL validation
**Vulnerability:** The `_is_safe_url` function in `appguardrail_core/controlplane.py` allowed SSRF bypasses because `urllib.parse.urlparse` parses URLs like `http://` or `http://user@` with a `None` hostname, which was converted to an empty string. Passing an empty string to `socket.getaddrinfo` resulted in a `socket.gaierror`, which was caught and ignored (to allow dummy test domains).
**Learning:** Defensive URL parsers must validate the presence of a hostname explicitly *before* attempting resolution. Missing hostnames are not "unresolvable dummy domains" but inherently invalid or potentially dangerous implicit addresses (e.g. localhost in some parsers).
**Prevention:** Always check `if not hostname: return False` after parsing and before DNS/IP resolution in security-sensitive SSRF prevention logic.
