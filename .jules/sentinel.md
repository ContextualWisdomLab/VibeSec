...
## 2025-02-28 - Unescaped severity property causing DOM XSS and Prototype vulnerabilities
**Vulnerability:** In `scanner/dashboard/console.html`, the `f.severity` property from unvalidated JSON was directly mapped into the `SEV` object without string normalization and injected directly into `innerHTML` without escaping, creating a DOM XSS vulnerability and potential Prototype manipulation vectors (e.g., using `__proto__`).
**Learning:** Even seemingly enum-like properties such as `severity` are untrusted if they come from user input (like a JSON findings payload). Trusting them blindly in dictionary lookups can trigger JS engine quirks, and interpolating them unescaped can lead to DOM XSS.
**Prevention:** When mapping untrusted string values to object keys in JavaScript (e.g., `SEV[f.severity]`), sanitize the input by coercing and normalizing it (e.g., `String(val).toUpperCase()`). Always use `Object.create(null)` for simple maps to prevent Object prototype lookup issues. Additionally, always use the `esc()` sanitizer for any dynamically rendered property injected into HTML.
