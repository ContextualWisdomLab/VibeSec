...
**Prevention:** When mapping untrusted string values to object keys in JavaScript (e.g., `SEV[f.severity]`), sanitize the input by coercing and normalizing it (e.g., `String(val).toUpperCase()`). Additionally, always use the `esc()` sanitizer for any dynamically rendered property injected into HTML.
