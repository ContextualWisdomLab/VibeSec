...
## 2026-08-05 - Fix Server-Side Request Forgery Bypass with Empty Hostnames
**Vulnerability:** The `_is_safe_url` functions in both `appguardrail_core/controlplane.py` and `scanner/cli/appguardrail.py` failed to reject URLs with empty hostnames (e.g., `http://` or `http://user@`). DNS resolution failures during validation were silently ignored, causing these URLs to bypass the checks and be treated as safe.
**Learning:** Functions that parse URLs and rely on DNS resolution (like `socket.getaddrinfo`) can raise exceptions when provided with empty or invalid hostnames. Catching and ignoring these exceptions broadly allows the underlying application to blindly process the unvalidated URL, potentially leading to SSRF if the client resolves it differently or forwards it to an internal service.
**Prevention:** Always explicitly check for and reject empty hostnames immediately after parsing a URL and before attempting DNS resolution or other network-level validation.
