...
## 2024-05-24 - DOM XSS and Prototype Pollution in Console
**Vulnerability:** XSS from untrusted data in numeric fields, unescaped ID attributes, and prototype pollution via uncoerced string keys mapping in vanilla JS.
**Learning:** When generating HTML without a framework, numeric properties (e.g. counts) must be strictly coerced to numbers using `Number()` to neutralize XSS payloads, and untrusted property mappings (like `SEV[severity]`) must be normalized (`String(val).toUpperCase()`) to avoid prototype pollution. Do not rely on implicit coercion. String properties (like IDs) injected into attributes must be escaped (`esc()`).
**Prevention:** Systematically apply `Number()` coercion for all numeric properties sourced from external payloads and use `String().toUpperCase()` for untrusted mapping keys before injection. Apply `esc()` to all dynamic attribute values.
