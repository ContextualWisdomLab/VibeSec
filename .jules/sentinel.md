...
## 2026-07-29 - Incomplete categorization boundaries in _finding_category
**Vulnerability:** Incomplete keyword mapping in `_finding_category` led to database-related secrets being categorized as `misconfig` rather than `secrets` in the reporting output.
**Learning:** Overly broad categories (such as reusing `_SENSITIVE_RULE_TOKENS` wholesale) misclassify unrelated rules containing provider names. On the other hand, narrow lists fail to capture credentials that don't follow the exact wording.
**Prevention:** Expanded the explicit secret boundaries in `_finding_category` to include `db-url`, `dsn`, `access-key`, `private-key`, and `supabase-service-role` while explicitly rejecting broad provider names like `github` to prevent misclassification of generic misconfigurations as secrets.
