...
## 2026-07-30 - Stored SSRF and Unhandled Parsing Exceptions Guardrail Check Bypass in Webhooks
**Vulnerability:** A previous patch meant to validate webhooks with `_is_safe_url` checking stored SSRF didn't protect against type confusion properly prior to parsing logic, leading to unhandled server errors on malicious input types when using `.get("url")`.
**Learning:** Checking for network input type constraints must occur closely combined with value assertion logic, and the endpoint itself needs to gracefully handle type errors instead of throwing `ValueError` natively into 500 server crash, or must catch raised ValueErrors natively inside HTTP route.
**Prevention:** Catch explicit `ValueError` internally raised from core validation handlers in API endpoints, and ensure `webhook_url` defaults strictly or captures correct object typing via explicit conditions in network endpoints.
