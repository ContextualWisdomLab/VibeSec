...
## 2026-07-29 - SSRF bypass via empty hostname
**Vulnerability:** Server-Side Request Forgery (SSRF) bypass due to `urllib.parse.urlparse` returning an empty hostname for URLs like `http://` or `http://user@`. The empty hostname was passed to `socket.getaddrinfo`, which raised an exception that was caught and ignored, leading to the URL being considered safe.
**Learning:** `urllib.parse.urlparse` can return an empty string for the hostname in certain edge cases, which bypassing IP and DNS resolution checks if the exceptions are swallowed.
**Prevention:** Always ensure the extracted hostname is not empty (`if not raw: return False`) before performing IP parsing or DNS resolution checks.
