...
## 2026-08-14 - Proxy Button Accessible Names
**Learning:** When replacing a native file input with a proxy button for better UX, duplicating the accessible name (e.g. keeping `aria-label="Upload findings file"` on a button whose visible text is already "Upload findings file") is redundant and violates the label-in-name principle, which can confuse screen reader users.
**Action:** When a proxy element's visible text perfectly describes the action, do not add a redundant `aria-label`. Rely on the visible text to provide the accessible name naturally.
